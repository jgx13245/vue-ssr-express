<template>
    <div class="wrap">
        <ul>
            <li v-for="(item,index) in pic" @click="showTitle(item.title)" :key="index">
                <img :src="item.image">
                <p>
                    {{item.title}}
                </p>
            </li>
        </ul>
         <router-link to="/pic"><button>跳转到pic</button></router-link>
          <router-link to="/test1"><button>跳转到test</button></router-link>
    </div>

</template>

<script>
    import Vue from 'vue'
    import { mapGetters} from 'vuex'
    import {PIC} from '../store/mutation-types'
    /*const computed = {
        ...mapGetters([
            'pic'
        ])
    }*/
    export default {
        data() {
            return {
              pic: []
            }
        },
        mounted() {
          // 第一种：存vuex
//            this.$store.dispatch('getData');
            // 获取数据
          // 第二种，直接页面获取
          this.api.getData().then((res) => {
            this.pic = res.data.data.liveWodList
          })
//            that.pic = response.data.data.liveWodList
        },
        methods: {
            showTitle(title) {
                alert(title)
            }
        },
        /*computed: {
          pic() {
            return this.$store.state.pic;
          }
        }*/
        // mounted() {
        //     Vue.axios.get('http://localhost:8090/data').then((response) => {
        //         const pic = response.data.data.liveWodList
        //         this.$store.commit(PIC.GET_DATA, pic)
        //     })
        // }
    }
</script>
<style>
    ul{
        list-style: none;
        display: flex;
        flex-direction: row;
        flex-wrap: wrap;
        margin: 0 auto;
        justify-content: center;
    }
    li{
        width: 380px;
        height: 270px;
        margin-right: 30px;
        background-color: #f2f2f2;
        cursor: pointer;
        margin-top: 10px;
    }

    img {
        width: 380px;
        height: 214px;
    }

    p {
        width: 380px;
        padding-left: 15px;
        font-size: 16px;
    }
</style>
